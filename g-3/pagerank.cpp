...
public:
  ...
  void init_vertex() {
    ...
    for (int32_t i=0; i<Vertex_Col_Len[_my_col]; i++) {
      if (this->_VertexOut[i] == 0)
        this->_VertexOut[i] = 1;
    }
    this->_VertexMsg.assign(Vertex_Col_Len[_my_col], 1.0/this->_VertexNum);
    this->_VertexMsgNew.assign(Vertex_Col_Len[_my_col], 0);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  // pg.init("/data/3/mp-3/eu/", 1070557254, 1600, 100);
  // pg.init("/data/3/mp-3/twitter/", 41652230, 300,  100);
  // pg.init("/data/3/mp-3/uk/", 787801471, 1000,  100);
  pg.init("/data/3/mp-3/webuk/", 133633040, 100, 100);
  pg.run();
  stop_time_app();
