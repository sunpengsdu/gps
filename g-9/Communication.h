...
void graphps_sendall(std::vector<T>& data_vector, int32_t changed_num, std::vector<bool>& data_state, int32_t start_col, int32_t end_col) {
  ...
  if (COMPRESS_NETWORK_LEVEL == 0) {
    for (int target_rank = start_col; target_rank <= end_col; target_rank++) {
...
void graphps_server_backend(std::vector<T>& VertexMsgNew, std::vector<T>& VertexMsg, int32_t id, std::vector<std::mutex>& locks) {
  ...
  while (1) {
    ...
    if (density >= DENSITY_VALUE) {
      ...
      int32_t k_end=0;
      int32_t id=0;
      if (get_col_id(start_id) == _my_rank) {
        k = 0;
        k_end = split_offset;
      } else {
        k = split_offset;
      ...
      for (; k<k_end; k++) {
        id = k+start_id-Vertex_Col_StartID[_my_rank];
        #ifdef PAGERANK
      ...
      for (; k<k_end; k++) {
        ...
        if (get_col_id(id) != _my_rank) {
          continue;
        }
        id = id-Vertex_Col_StartID[_my_rank];
        #ifdef PAGERANK
